...
In the current step we will migrate the application database located on the **PostgreSQL** server 
to the new schema version, separating adding an `acceptable` column to the existing `air_quality_records` table
and filling it with generated values.
...
2. Build **Docker** images of distribution files for the new versions of these packages:
   ...
   ```shell
   docker build -f pkgs/core/dist.Dockerfile -t wwweather/dists/core:0.3.0 pkgs/core
   docker build -f pkgs/data-csv/dist.Dockerfile -t wwweather/dists/data-csv:0.3.0 pkgs/data-csv
   docker build -f pkgs/data-sqlalchemy/dist.Dockerfile -t wwweather/dists/data-sqlalchemy:0.3.0 pkgs/data-sqlalchemy
   docker build -f cli/dist.Dockerfile -t wwweather/dists/cli:0.3.0 cli
   ```
...
4. Run **Docker Compose** `postgres-upgrade` profile to apply migration up to the `model-0.3.0` revision:
